  ...
  useEffect(() => {
	...
	const renderer = ({ 
		...
		if (completed){
			...
			return (
				<div className="flex justify-center items-center space-x-4">
					<CountdownItem value={formatTime(days)} label="DAYS" />
					<CountdownItem value={formatTime(hours)} label="HRS" />
					<CountdownItem value={formatTime(minutes)} label="MIN" />
					<CountdownItem value={formatTime(seconds)} label="SEC" />
				</div>
				);
		};
	};
